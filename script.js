...
const activeChain = "binance"; // تم التغيير إلى Binance Smart Chain
...
const initializeSDK = async (signer) => {
    ...
    try {
        // Use the globally available ThirdwebSDK
        sdk = new thirdweb.ThirdwebSDK(signer, activeChain);
    ...
    const connectWallet = async () => {
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            try {
                ...
                // Request accounts and check network
                const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
                const chainId = await window.ethereum.request({ method: 'eth_chainId' });

                const bscChainId = '0x38'; // Chain ID for BSC Mainnet in hexadecimal (56 in decimal)
                ...
                if (chainId !== bscChainId) {
                    try {
                        ...
                        // Re-fetch chainId after switch attempt
                        const newChainId = await window.ethereum.request({ method: 'eth_chainId' });
                        if (newChainId !== bscChainId) {
                            alert("Please switch to Binance Smart Chain (BSC) Mainnet in your wallet.");
                            throw new Error("Wrong network. Please switch to BSC Mainnet.");
                        }
                    } catch (switchError) {
                        ...
                        if (switchError.code === 4902) {
                            try {
                                ...
                                throw addError; // Re-throw to stop connection process
                            }
                        } else if (switchError.code === 4001) {
                            alert("Network switch rejected by user.");
                            throw switchError; // Re-throw to stop connection process
                        } else {
                            console.error("Error switching network:", switchError);
                            alert("Failed to switch to Binance Smart Chain. Please check your wallet.");
                            throw switchError; // Re-throw to stop connection process
                        }
                    }
                }
                ...
                window.ethereum.on('chainChanged', async (newChainId) => {
                    ...
                    if (newChainId !== bscChainId) {
                        alert("You switched to a different network. Please switch back to Binance Smart Chain (BSC) Mainnet.");
                        disconnectWallet(); // Disconnect if not on BSC
                    } else {
                        // Re-initialize SDK with the current signer if available
                        if (walletConnected && currentAccount) {
                            const newSigner = provider.getSigner();
                            await initializeSDK(newSigner);
                            alert("Successfully switched to BSC Mainnet.");
                        }
                    }
                    updateWalletStatus();
                });
                ...
                if (error.code === 4001) {
                    ...
                    alert("Failed to connect wallet. Please make sure MetaMask or Trust Wallet is installed and unlocked, and you are on the correct network (Binance Smart Chain). Check console for details.");
                }
                updateWalletStatus();
            }
        } else {
